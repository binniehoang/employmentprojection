...
def main():
	...
	X = pd.read_csv(os.path.join('model_data', 'selected_features.csv'))
	y = pd.read_csv(os.path.join('model_data', 'target.csv')).squeeze()
	...
	try:
		importance_path = os.path.join('model_data', 'feature_importances.csv')
	...
	# added error handling for file writing
	try:
		logs_dir = os.path.join('model_data', 'logs')
